...
@TestHTTPEndpoint(LongEntityResource.class)
@TestMethodOrder(MethodOrderer.OrderAnnotation.class)
@QuarkusTestResource(H2DatabaseTestResource.class)
public class LongEntityResourceIT {

    private EntityUsingLong validEntity;
    private EntityUsingLong invalidEntity;

    // Should be the JPA Generated ID since JPA (creates INCREMENT with a value of 50 x 4 manually inserted records)
    private final Long insertedId = 152L;

    @BeforeAll
    public void init() {
        this.invalidEntity = new EntityUsingLong();
        invalidEntity.setOtherUniqueItem(0L);
        invalidEntity.setNonUniqueText("");
        this.validEntity = new EntityUsingLong();
        validEntity.setOtherUniqueItem(152L);
    ...
    public void valid_post_returns_201_with_uri() {
        given().contentType(ContentType.JSON).body(validEntity).when().post().then().statusCode(is(201))
                .header("Location", containsString("/sequences/" + insertedId));
    }

    @Test
    @Order(3)
    public void duplicate_post_returns_409() {
        given().contentType(ContentType.JSON).body(validEntity).when().post().then().statusCode(is(409))
    ...
    public void invalid_put_returns_400() {
        EntityUsingLong invalidEdit = this.validEntity;
        invalidEdit.setLongAsId(insertedId);
    ...
    public void put_with_valid_entity_returns_200_with_json_payload() {
        EntityUsingLong validEdit = this.validEntity;
        validEdit.setLongAsId(insertedId);
